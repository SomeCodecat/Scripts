...
sudo cp -a "$(dirname "$0")/backup_stacks.conf" "$DEST_DIR/backup_stacks.conf"
sudo chmod 755 "$DEST_DIR/backup_stacks.sh"
sudo chmod 600 "$DEST_DIR/backup_stacks.conf"
sudo mkdir -p "$BACKUP_DIR"
sudo chown root:root "$DEST_DIR" "$DEST_DIR/backup_stacks.sh" "$DEST_DIR/backup_stacks.conf" "$BACKUP_DIR"
sudo chmod 700 "$BACKUP_DIR"

echo "Installed. Edit $DEST_DIR/backup_stacks.conf and set PORTAINER_URL and PORTAINER_API_KEY." 
